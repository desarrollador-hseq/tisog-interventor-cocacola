...
>[] = [
  ...
  {
    accessorKey: "proposedClosureDate",
    enableColumnFilter: false,
    accessorFn: (value) =>
      value.proposedClosureDate
        ? format(value.proposedClosureDate, "P", { locale: es })
        : "",
    header: ({ column }) => {
      return (
        <Button
    ...
    cell: ({ row }) => {
      const date = row.original?.proposedClosureDate;
      const dateFormated = row.original?.proposedClosureDate
        ? format(row.original?.proposedClosureDate, "P", { locale: es })
        : "";
      return <div className="">{dateFormated}</div>;
    },
  },
  {
    accessorKey: "actualClosureDate",
    enableColumnFilter: false,
    accessorFn: (value) =>
      value.actualClosureDate
        ? format(value.actualClosureDate, "P", { locale: es })
        : "",
    header: ({ column }) => {
      return (
        <Button
    ...
    cell: ({ row }) => {
      const date = row.original?.actualClosureDate;
      const dateFormated = row.original?.actualClosureDate
        ? format(row.original?.actualClosureDate, "P", { locale: es })
        : "";
      return <div className="">{dateFormated}</div>;
    },
  },
  ...
  },
  {
    ...
    accessorFn: (value) => {
      const turn = value.typeAction;
      return typeActionOptions.find((d) => d.value === turn) || "No asignado";
    },
    enableColumnFilter: false,
    ...
    cell: ({ row }) => {
      const turn = row.original?.typeAction;
      const sourceEsp = typeActionOptions.find((d) => d.value === turn);
      return (
        <div className="capitalize">
          {sourceEsp ? sourceEsp.label : "No asignado"}
        </div>
      );
    },
  },
  ...
  {
    accessorKey: "actionInmediate",
    accessorFn: (value) => value.actionInmediate || "",
    header: ({ column }) => {
      return (
        <Button
          variant="ghost"
    ...
    cell: ({ row }) => {
      const turn = row.original?.actionInmediate || "";

      return <div className="">{turn}</div>;
    },
  },
  {
    accessorKey: "actionToTake",
    accessorFn: (value) => value.actionToTake || "",
    header: ({ column }) => {
      return (
        <Button
          variant="ghost"
    ...
    cell: ({ row }) => {
      const turn = row.original?.actionToTake || "";
